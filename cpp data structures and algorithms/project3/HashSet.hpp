...
public:
    ...
    void empty(std::shared_ptr<Node>* table); //sets every node to nullptr, DOES NOT DEALLOCATE

    float load_factor();

    void rehash();

    void smol_add(const ElementType& element, std::shared_ptr<Node>* target_table);
...
HashSet<ElementType>::HashSet(HashFunction hashFunction)
    : hashFunction{hashFunction}, table{new std::shared_ptr<Node>[DEFAULT_CAPACITY]}, capacity{DEFAULT_CAPACITY}, _size{0}, rehash_count{0}
{
	empty(table);
}
...
HashSet<ElementType>::HashSet(const HashSet& s)
    : hashFunction{impl_::HashSet__undefinedHashFunction<ElementType>}, table{new std::shared_ptr<Node>[s.capacity]}, capacity{s.capacity}, _size{s.size()}, rehash_count{0}
{
	hashFunction = s.hashFunction;
	empty(table);

	for (int i = 0; i < capacity; i++){
		std::shared_ptr<Node> navigator = s.table[i];
		std::shared_ptr<Node> pointer = nullptr;
		while (navigator != nullptr){
			if (table[i] == nullptr){
				table[i] = std::shared_ptr<Node>{new Node{navigator->value, nullptr}};
				pointer = table[i];
			}
			else{
				pointer->next = std::shared_ptr<Node>{new Node{navigator->value, nullptr}};
...
HashSet<ElementType>& HashSet<ElementType>::operator=(const HashSet& s)
{
	if ( &s!= this){
		...
		std::shared_ptr<Node>* temp_table = new std::shared_ptr<Node>[s.capacity];
		empty(temp_table);
		...
		for (int i = 0; i < s.capacity; i++){
			...
			while (navigator != nullptr){
				if (temp_table[i] == nullptr){
					temp_table[i] = std::shared_ptr<Node>{new Node{navigator->value, nullptr}};
...
template<typename ElementType>
void HashSet<ElementType>::empty(std::shared_ptr<Node>* table){
	for (int i = 0; i < capacity; i ++){
		table[i] = nullptr;
	}
}
...
void HashSet<ElementType>::rehash(){
	...
	capacity *= 2;
	std::shared_ptr<Node>* temp_table = new std::shared_ptr<Node>[capacity];

	empty(temp_table);

	for (int i = 0; i < temp_capacity; i++){

		std::shared_ptr<Node> head = table[i];
...
template<typename ElementType>
void HashSet<ElementType>::smol_add(const ElementType& element, std::shared_ptr<Node>* target_table){
	unsigned int index = hashFunction(element)%capacity;
	// std::cout<<"hash value for: " << element << " is "<< hashFunction(element) << std::endl;

	if (target_table[index] == nullptr){
		target_table[index] = std::shared_ptr<Node>{new Node{element, nullptr}};
		_size++;
	}
		...
		std::shared_ptr<Node> head = target_table[index];
		...
		head->next = std::shared_ptr<Node>{new Node{element, nullptr}};
		_size++;
	}
