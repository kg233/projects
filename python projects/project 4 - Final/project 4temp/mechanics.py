...
class GameField:
    ...
    def age(self):
        ...
        elif not self.hasfaller:
            self.list = remove_star(self.list)
            return self.list  
    ...
    def _freeze(self) -> 'dlist':
        ...
        if type(self.faller) == Lander:
            ...
            self.block_field = BlockField(self.list)
            
            self.block_list = self.block_field.block_list
...
def set_size(row, column, ask_type) -> list:
    '''ask user for row and column, return a list based on input'''
    temp = create_empty_list(row, column)
    if ask_type == 'EMPTY':
        return temp
    wordlist = []
    for row_ind in range(row):
        ...
        if len(s) != column:
            ...
            return
        wordlist.append(split_to_list(s))
    expanded_word = transform_list_perspective(wordlist)
...
def perform_rotation(gameF) -> GameField:
            ...
            if dlist[column][ind] != '   ':
                if ind + 2 <= len(dlist[0]) - 1:
                    ...
                    dlist[column][ind][1] == dlist[column][ind+2][1]:
                        dlist[column][ind] = '*{}*'.format(dlist[column][ind][1])
                        dlist[column][ind+1] = '*{}*'.format(dlist[column][ind+1][1])
                        dlist[column][ind+2] = '*{}*'.format(dlist[column][ind+2][1])
    return dlist
                    ...
                    if ind2 + 2 <= len(dlist[0]) - 1:
                        if dlist[ind1][ind2][1] == dlist[ind1 + 1][ind2 + 1][1] and \
                           dlist[ind1][ind2][1] == dlist[ind1 + 2][ind2 + 2][1]:
                            dlist[ind1][ind2] = '*{}*'.format(dlist[ind1][ind2][1])
                            dlist[ind1+1][ind2+1] = '*{}*'.format(dlist[ind1+1][ind2+1][1])
                            dlist[ind1+2][ind2+2] = '*{}*'.format(dlist[ind1+2][ind2+2][1])
                    if ind2 - 2 >= 0:
                        if dlist[ind1][ind2][1] == dlist[ind1 + 1][ind2 - 1][1] and \
                           dlist[ind1][ind2][1] == dlist[ind1 + 2][ind2 - 2][1]:
                            dlist[ind1][ind2] = '*{}*'.format(dlist[ind1][ind2][1])
...
def check_all(dlist) -> 'dlist':
    '''check for horizontal, vertical, and diagonal'''
    return check_diagonal(check_horizontal(check_vertical(dlist)))
