...
public:
    ...
    // representing a function that takes a reference to a const
...
public:
    ...
    //created functions
    unsigned int get_hashID(const std::string& key) const;

    void visualize();

    void rehasher(unsigned int new_cap);
...
public:
    void copy_table_from(Node** const orig_table, unsigned int cap);

    void delete_one_bucket(Node* head);
